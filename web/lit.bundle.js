...
// web/components.js
var NotemapPanel = class extends i4 {
  static styles = i`
    :host { all: initial; }
    .toggle {
  ...
  constructor() {
    ...
    this._rect = null;
    try {
      const r4 = JSON.parse(localStorage.getItem("dsh.notemap.rect") || "null");
      if (r4 && r4.left != null && r4.top != null && r4.w >= 320 && r4.h >= 240) this._rect = r4;
    } catch {
    }
  ...
  firstUpdated() {
    ...
    const save = () => {
      try {
        localStorage.setItem("dsh.notemap.rect", JSON.stringify({ left: win.offsetLeft, top: win.offsetTop, w: win.offsetWidth, h: win.offsetHeight }));
      } catch {
      }
    };
    let drag = null;
    head.addEventListener("pointerdown", (e4) => {
      drag = { dx: e4.clientX - win.offsetLeft, dy: e4.clientY - win.offsetTop, moved: false };
      head.setPointerCapture(e4.pointerId);
    });
    head.addEventListener("pointermove", (e4) => {
      if (!drag) return;
      win.style.left = Math.min(Math.max(0, e4.clientX - drag.dx), window.innerWidth - 40) + "px";
      win.style.top = Math.min(Math.max(0, e4.clientY - drag.dy), window.innerHeight - 40) + "px";
      drag.moved = true;
    });
    ...
    let resize = null;
    grip.addEventListener("pointerdown", (e4) => {
      resize = { x: e4.clientX, y: e4.clientY, w: win.offsetWidth, h: win.offsetHeight };
      grip.setPointerCapture(e4.pointerId);
      e4.stopPropagation();
    });
    grip.addEventListener("pointermove", (e4) => {
      if (!resize) return;
      win.style.width = Math.max(320, Math.min(window.innerWidth - win.offsetLeft, resize.w + (e4.clientX - resize.x))) + "px";
      win.style.height = Math.max(240, Math.min(window.innerHeight - win.offsetTop, resize.h + (e4.clientY - resize.y))) + "px";
    });
